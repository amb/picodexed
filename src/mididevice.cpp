...
CMIDIDevice::CMIDIDevice(CPicoDexed* pSynth) : m_pSynth(pSynth) {
...
void CMIDIDevice::SetChannel(uint8_t ucChannel) { m_ucChannel = ucChannel; }

uint8_t CMIDIDevice::GetChannel(void) { return m_ucChannel; }
    ...
    if(m_Message.valid && !m_Message.processed && m_Message.type != ActiveSensing) {
        if(m_Message.type == SystemExclusive) {
            fprintf(stderr, "T:%02x L:%d 0x%02x%02x%02x%02x%02x%02x%02x%02x\n", m_Message.type,
                    m_Message.getSysExSize(), m_Message.sysex[0], m_Message.sysex[1], m_Message.sysex[2],
                    m_Message.sysex[3], m_Message.sysex[4], m_Message.sysex[5], m_Message.sysex[6], m_Message.sysex[7]);
        } else {
            fprintf(stderr, "T:%02x C:%02x D:%02x D:%02x\n", m_Message.type, m_Message.channel, m_Message.d1,
                    m_Message.d2);
        }
    }
#endif
